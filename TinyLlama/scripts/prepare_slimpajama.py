...
def prepare_full(
    ...
    for filepath in filenames:
        ...
        filename_hash = hashlib.md5(filepath.encode()).hexdigest()[:8]
        output_bin = destination_path / f"{split}_slimpajama_{process_id}_{filename_hash}_0.bin"

        if output_bin.exists():
            print(f"Skipping {filepath}, already processed.")
            continue
        with zstd.open(open(filepath, "rb"), "rt", encoding="utf-8") as f:
            for row in tqdm(f, desc=f"{filepath[-30:]}"):
                try:
                    record = json.loads(row)
                    if record["meta"]["redpajama_set_name"] == "RedPajamaGithub":
                        continue
                    text_ids = tokenizer.encode(record["text"])
                    builder.add_array(np.array(text_ids, dtype=builder.dtype))
                except Exception as e:
                    print(f"Skipping one row due to error: {e}")
